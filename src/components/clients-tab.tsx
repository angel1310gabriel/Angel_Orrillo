...
const sC=(s:number|null)=>{if(s===null)return{bg:'bg-slate-100',tx:'text-slate-600',bd:'border-slate-200',lb:'Sin score',br:'bg-slate-400'};if(s<=30)return{bg:'bg-red-100',tx:'text-red-700',bd:'border-red-200',lb:'Malo',br:'bg-red-500'};if(s<=60)return{bg:'bg-amber-100',tx:'text-amber-700',bd:'border-amber-200',lb:'Regular',br:'bg-amber-500'};return{bg:'bg-emerald-100',tx:'text-emerald-700',bd:'border-emerald-200',lb:'Bueno',br:'bg-emerald-500'}};
const lB=(st:string)=>{const m:Record<string,[string,string]>={active:['bg-emerald-100 text-emerald-800 border-emerald-200','Activo'],mora:['bg-red-100 text-red-800 border-red-200','Mora'],completed:['bg-teal-100 text-teal-800 border-teal-200','Pagado'],paid:['bg-teal-100 text-teal-800 border-teal-200','Pagado'],cancelled:['bg-slate-100 text-slate-600 border-slate-200','Cancelado'],refinanced:['bg-amber-100 text-amber-800 border-amber-200','Refinanciado']};return m[st]||['bg-slate-100 text-slate-600 border-slate-200',st]};
const dSB=(t:string)=>{if(t==='dni'||t==='peruano')return['DNI','bg-blue-50 text-blue-700 border-blue-200'];if(t==='carnet_extranjeria'||t==='extranjero')return['CE','bg-purple-50 text-purple-700 border-purple-200'];if(t==='pasaporte')return['PAS','bg-amber-50 text-amber-700 border-amber-200'];return['DOC','bg-slate-50 text-slate-600 border-slate-200']};
...
const avt=(c:CWS)=>c.stats.hasMora?['bg-red-100','text-red-600']:c.stats.activeLoans>0?['bg-emerald-100','text-emerald-600']:['bg-teal-100','text-teal-600'];
...
<div className="relative"><Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-slate-400"/><Input placeholder="Buscar..." value={search} onChange={e=>{setSearch(e.target.value);setPage(1)}} className="pl-9 w-full sm:w-64 bg-white border-slate-200"/></div>
<Select value={zoneF} onValueChange={v=>{setZoneF(v);setPage(1)}}><SelectTrigger className="w-full sm:w-44 bg-white border-slate-200"><MapPin className="h-4 w-4 mr-2 text-slate-400"/><SelectValue placeholder="Zonas"/></SelectTrigger><SelectContent><SelectItem value="all">Todas</SelectItem>{zones.map(z=><SelectItem key={z.id} value={z.id}>{z.name}</SelectItem>)}</SelectContent></Select>
</div>
...
<div className="flex items-center gap-2 flex-wrap"><h4 className="font-semibold text-slate-900 truncate">{c.name}</h4><Badge variant="outline" className={`text-[10px] px-1.5 py-0 h-5 ${dc}`}>{dl}</Badge>{c.stats.hasMora&&<Badge variant="outline" className="bg-red-100 text-red-800 border-red-200 text-xs"><AlertTriangle className="h-3 w-3 mr-1"/>Mora</Badge>}</div>
<div className="flex items-center gap-3 text-xs text-slate-500 mt-0.5 flex-wrap"><span className="flex items-center gap-1"><FileText className="h-3 w-3"/>{dn}</span><span className="flex items-center gap-1"><Phone className="h-3 w-3"/>{c.phone}</span>{c.zone&&<span className="flex items-center gap-1"><MapPin className="h-3 w-3"/>{c.zone.name}</span>}<Badge variant="outline" className={`${sc.bg} ${sc.tx} ${sc.bd} text-[10px] font-semibold hidden sm:inline-flex`}>{c.creditScore??'N/A'} {sc.lb}</Badge></div>
</div>
<div className="hidden md:flex items-center gap-4 text-center shrink-0"><div><p className="text-[10px] text-slate-400">Préstamos</p><p className="font-bold text-sm">{c.stats.activeLoans}/{c.stats.totalLoans}</p></div><div><p className="text-[10px] text-slate-400">Prestado</p><p className="font-bold text-sm">{fC(c.stats.totalLoaned)}</p></div><div><p className="text-[10px] text-slate-400">Pagado</p><p className="font-bold text-sm text-emerald-600">{fC(c.stats.totalPaid)}</p></div></div>
...
{totalP>1&&<div className="flex items-center justify-center gap-2 pt-4"><Button variant="outline" size="sm" disabled={page<=1} onClick={()=>setPage(page-1)}><ChevronLeft className="h-4 w-4"/></Button><span className="text-sm text-slate-600">{page}/{totalP}</span><Button variant="outline" size="sm" disabled={page>=totalP} onClick={()=>setPage(page+1)}><ChevronRight className="h-4 w-4"/></Button></div>}
</div>}
...
<span className={`text-[11px] ${sel?'text-emerald-600':'text-slate-400'}`}>{dt.dg} dígitos</span>
</button>)})}</div>
</div>
{/* Doc Number */}
<div className="space-y-2">
...
<div className={`px-3 py-2 flex items-center gap-2 ${vRes.found?'bg-amber-50 border-b border-amber-200':'bg-emerald-50 border-b border-emerald-200'}`}>{vRes.found?<><AlertTriangle className="h-4 w-4 text-amber-600"/><span className="font-semibold text-amber-700">Registros encontrados</span></>:<><CheckCircle2 className="h-4 w-4 text-emerald-600"/><span className="font-semibold text-emerald-700">Sin registros previos</span></>}</div>
{vRes.found&&<div className="p-2 space-y-1.5 max-h-36 overflow-y-auto bg-white">
{vRes.results.clients.map(cr=><div key={cr.id} className="flex items-center gap-2 p-2 bg-slate-50 rounded-md text-xs"><User className="h-3.5 w-3.5 text-emerald-600"/><span className="font-medium truncate">{cr.name}</span><span className="text-slate-500">{cr.phone}</span><Badge variant="outline" className={`text-[10px] ml-auto ${cr.hasActiveLoans?'bg-amber-50 text-amber-700 border-amber-200':'bg-emerald-50 text-emerald-700 border-emerald-200'}`}>{cr.hasActiveLoans?'Con préstamo':'Cliente'}</Badge></div>)}
{vRes.results.staff.map(sr=><div key={sr.id} className="flex items-center gap-2 p-2 bg-slate-50 rounded-md text-xs"><Shield className="h-3.5 w-3.5 text-purple-600"/><span className="font-medium truncate">{sr.name}</span><span className="text-slate-500">{sr.phone}</span><Badge variant="outline" className="text-[10px] ml-auto bg-purple-50 text-purple-700 border-purple-200">{sr.role}</Badge></div>)}
</div>}</div>}
</div>
{/* Name */}
<div className="space-y-2">
<Label htmlFor="nm" className="text-sm font-semibold text-slate-700">Nombre completo <span className="text-red-500">*</span></Label>
<div className="relative"><Input id="nm" placeholder="Juan Pérez García" value={fName} onChange={e=>setFName(e.target.value)} className={`bg-white h-11 pr-10 ${fName?(nOk?'border-emerald-400 focus-visible:ring-emerald-400':'border-red-400 focus-visible:ring-red-400'):''}`}/>{fName&&<div className="absolute right-3 top-1/2 -translate-y-1/2">{nOk?<CheckCircle2 className="h-4 w-4 text-emerald-500"/>:<XCircle className="h-4 w-4 text-red-400"/>}</div>}</div>
</div>
{/* Phone */}
<div className="space-y-2">
<Label htmlFor="ph" className="text-sm font-semibold text-slate-700">Teléfono <span className="text-red-500">*</span></Label>
<div className="flex items-center gap-2">
<div className="relative flex-1">
<Input id="ph" placeholder="999888777" value={fPh} onChange={e=>onPhC(e.target.value)} maxLength={9} className={`bg-white h-11 pr-20 font-mono tracking-wider ${vB(phV)}`}/>
<div className={`absolute right-3 top-1/2 -translate-y-1/2 flex items-center gap-1.5 ${phV?.[0]?'text-emerald-600':'text-slate-400'}`}><span className="text-xs font-mono">{fPh.length}/9</span>{phV&&(phOk?<BadgeCheck className="h-5 w-5 text-emerald-500"/>:phV[0]?<CheckCircle2 className="h-4 w-4 text-emerald-500"/>:<XCircle className="h-4 w-4 text-red-400"/>)}</div>
</div>
<Button type="button" variant="outline" className={`h-11 shrink-0 px-4 font-semibold ${phOk?'bg-emerald-50 border-emerald-300 text-emerald-700':pOk?'border-emerald-300 text-emerald-700':'border-slate-200 text-slate-400'}`} onClick={verPh} disabled={!pOk}>{phOk?<><CheckCheck className="h-4 w-4 mr-1.5"/>Verificado</>:<><BadgeCheck className="h-4 w-4 mr-1.5"/>Verificar</>}</Button>
</div>
{phV&&<p className={`text-xs ${phV[0]?'text-emerald-600':'text-red-500'}`}>{phV[1]}</p>}
</div>
{/* Address */}
<div className="space-y-2">
<Label htmlFor="ad" className="text-sm font-semibold text-slate-700">Dirección <span className="text-slate-400 font-normal">(opcional)</span></Label>
<Input id="ad" placeholder="Av. Principal 123" value={fAddr} onChange={e=>setFAddr(e.target.value)} className="bg-white h-11"/>
</div>
{/* Zone */}
<div className="space-y-2">
<Label className="text-sm font-semibold text-slate-700">Zona <span className="text-slate-400 font-normal">(opcional)</span></Label>
<Select value={fZone} onValueChange={setFZone}><SelectTrigger className="bg-white border-slate-200 h-11"><MapPin className="h-4 w-4 mr-2 text-slate-400"/><SelectValue placeholder="Seleccionar zona"/></SelectTrigger><SelectContent>{zones.map(z=><SelectItem key={z.id} value={z.id}>{z.name}</SelectItem>)}</SelectContent></Select>
</div>
{/* Score */}
<div className="space-y-3">
<div className="flex items-center justify-between"><Label className="text-sm font-semibold text-slate-700">Score crediticio</Label><Badge variant="outline" className={`${sC(fScore[0]).bg} ${sC(fScore[0]).tx} ${sC(fScore[0]).bd} font-semibold`}>{fScore[0]} — {sC(fScore[0]).lb}</Badge></div>
...
<DialogFooter className="px-6 py-4 border-t border-slate-100 bg-gradient-to-r from-slate-50 to-white">
...
<AlertDialogContent>
<AlertDialogHeader>
<AlertDialogTitle className="flex items-center gap-2"><div className="w-8 h-8 rounded-lg bg-red-100 flex items-center justify-center"><AlertTriangle className="h-4 w-4 text-red-600"/></div>Eliminar Cliente</AlertDialogTitle>
<AlertDialogDescription>¿Eliminar a <strong>{delC?.name}</strong>? No se puede deshacer.</AlertDialogDescription>
</AlertDialogHeader>
{delC&&delC.stats.activeLoans>0&&<Alert className="bg-red-50 border-red-200"><AlertTriangle className="h-4 w-4 text-red-600"/><AlertDescription className="text-red-700">Tiene {delC.stats.activeLoans} préstamo(s) activo(s).</AlertDescription></Alert>}
<AlertDialogFooter>
...
<div className="flex items-center justify-between mb-2"><span className="text-sm font-medium text-slate-700">Score</span><Badge variant="outline" className={`font-bold ${sc.bg} ${sc.tx} ${sc.bd}`}>{c.creditScore??'N/A'} — {sc.lb}</Badge></div>
...
<div className="space-y-2">
<h3 className="text-sm font-semibold text-slate-700 flex items-center gap-2"><Phone className="h-4 w-4 text-emerald-500"/>Información</h3>
<div className="flex items-center gap-3 p-3 bg-slate-50 rounded-lg"><IdCard className="h-4 w-4 text-slate-400"/><div><p className="text-xs text-slate-400">Documento</p><p className="text-sm font-medium">{dl} {dn}</p></div></div>
<div className="flex items-center gap-3 p-3 bg-slate-50 rounded-lg"><Phone className="h-4 w-4 text-slate-400"/><div><p className="text-xs text-slate-400">Teléfono</p><p className="text-sm font-medium">{c.phone}</p></div></div>
{c.address&&<div className="flex items-center gap-3 p-3 bg-slate-50 rounded-lg"><MapPin className="h-4 w-4 text-slate-400"/><div><p className="text-xs text-slate-400">Dirección</p><p className="text-sm font-medium">{c.address}</p></div></div>}
{c.zone&&<div className="flex items-center gap-3 p-3 bg-slate-50 rounded-lg"><MapPin className="h-4 w-4 text-slate-400"/><div><p className="text-xs text-slate-400">Zona</p><p className="text-sm font-medium">{c.zone.name}</p></div></div>}
<div className="flex items-center gap-3 p-3 bg-slate-50 rounded-lg"><Calendar className="h-4 w-4 text-slate-400"/><div><p className="text-xs text-slate-400">Registro</p><p className="text-sm font-medium">{fD(c.createdAt)}</p></div></div>
</div>
<Separator/>
<div className="space-y-3">
<h3 className="text-sm font-semibold text-slate-700 flex items-center gap-2"><TrendingUp className="h-4 w-4 text-emerald-500"/>Financiero</h3>
<div className="grid grid-cols-2 gap-3">
<div className="p-3 bg-emerald-50 rounded-lg text-center border border-emerald-100"><p className="text-xs text-emerald-600">Prestado</p><p className="text-lg font-bold text-emerald-700">{fC(c.stats.totalLoaned)}</p></div>
<div className="p-3 bg-teal-50 rounded-lg text-center border border-teal-100"><p className="text-xs text-teal-600">Pagado</p><p className="text-lg font-bold text-teal-700">{fC(c.stats.totalPaid)}</p></div>
<div className="p-3 bg-slate-50 rounded-lg text-center border border-slate-100"><p className="text-xs text-slate-600">Préstamos</p><p className="text-lg font-bold text-slate-700">{c.stats.totalLoans}</p></div>
<div className={`p-3 rounded-lg text-center border ${c.stats.hasMora?'bg-red-50 border-red-100':'bg-emerald-50 border-emerald-100'}`}><p className={`text-xs font-medium ${c.stats.hasMora?'text-red-600':'text-emerald-600'}`}>Estado</p><p className={`text-lg font-bold ${c.stats.hasMora?'text-red-700':'text-emerald-700'}`}>{c.stats.hasMora?'En Mora':'Al día'}</p></div>
</div>
</div>
{c.guarantors.length>0&&<><Separator/><div className="space-y-2"><h3 className="text-sm font-semibold text-slate-700 flex items-center gap-2"><Shield className="h-4 w-4 text-emerald-500"/>Avalistas ({c.guarantors.length})</h3>{c.guarantors.map(g=><div key={g.id} className="flex items-center gap-3 p-3 bg-slate-50 rounded-lg"><div className="w-8 h-8 rounded-full bg-amber-100 flex items-center justify-center"><User className="h-4 w-4 text-amber-600"/></div><div><p className="text-sm font-medium">{g.name}</p>{g.phone&&<p className="text-xs text-slate-500">{g.phone}</p>}</div></div>)}</div></>}
{c.loans.length>0&&<><Separator/><div className="space-y-2"><h3 className="text-sm font-semibold text-slate-700 flex items-center gap-2"><CreditCard className="h-4 w-4 text-emerald-500"/>Préstamos ({c.loans.length})</h3>{c.loans.map(l=>{const[cn,lb]=lB(l.status),pct=l.totalAmount>0?Math.round((l.amountPaid/l.totalAmount)*100):0;return<div key={l.id} className="p-3 bg-slate-50 rounded-lg space-y-2"><div className="flex items-center justify-between"><Badge variant="outline" className={`text-xs ${cn}`}>{lb}</Badge><span className="text-sm font-bold">{fC(l.amount)}</span></div><div className="flex justify-between text-xs text-slate-500"><span>Pagado: {fC(l.amountPaid)}</span><span>{pct}%</span></div><Progress value={pct} className="h-1.5"/></div>})}</div></>}
