...
<div className="flex items-center gap-2 text-center shrink-0"><div><p className="text-[10px] text-slate-400">Préstamos</p><p className="font-bold text-sm">{c.stats.totalLoans}</p></div><div><p className="text-[10px] text-slate-400">Prestado</p><p className="font-bold text-sm">{fC(c.stats.totalLoaned)}</p></div><div><p className="text-[10px] text-slate-400">Pagado</p><p className="font-bold text-sm text-emerald-600 dark:text-emerald-300">{fC(c.stats.totalPaid)}</p></div></div>
<div className="flex items-center gap-1 shrink-0" onClick={e=>e.stopPropagation()}>
{c.stats.activeLoans>0&&<Button className="h-8 w-8 bg-emerald-600 hover:bg-emerald-700 text-white border-0" size="icon" onClick={()=>{window.location.href='?tab=payments'}}><DollarSign className="h-4 w-4"/></Button>}
...
<div className="flex items-center gap-3">{c.photoUrl?<img src={c.photoUrl} alt={c.name} className="w-12 h-12 rounded-full object-cover border-2 border-emerald-200"/>:<div className={`w-12 h-12 rounded-full flex items-center justify-center ${ab}`}><User className={`h-6 w-6 ${at}`}/></div>}<div><SheetTitle className="text-lg">{c.name}</SheetTitle><SheetDescription className="flex items-center gap-2 mt-0.5"><Badge variant="outline" className={`text-[10px] px-1.5 py-0 h-5 ${dc}`}>{dl}</Badge>{dn}{c.status!==null&&<Badge variant="outline" className={`text-[10px] px-1.5 py-0 h-5 ${c.status==='active'?'bg-emerald-50 dark:bg-emerald-950/50 text-emerald-700 dark:text-emerald-300 border-emerald-200':'bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400 border-slate-200'}`}>{c.status==='active'?'Activo':'Inactivo'}</Badge>}</SheetDescription></div></div>
</SheetHeader>
...
{c.creditScoreLabel&&<div className="mt-2 text-xs text-slate-500 dark:text-slate-400 text-center">Etiqueta: <span className="font-semibold text-slate-700 dark:text-slate-300">{c.creditScoreLabel}</span></div>}
</CardContent></Card>
<div className="space-y-2">
...
{c.collectorId&&<div className="flex items-center gap-3 p-3 bg-slate-50 dark:bg-slate-800/50 rounded-lg"><User className="h-4 w-4 text-slate-400"/><div><p className="text-xs text-slate-400">Cobrador ID</p><p className="text-sm font-medium font-mono text-xs">{c.collectorId.slice(0,8)}...</p></div></div>}
</div>
<Separator/>
<div className="space-y-3">
...
toast({title:'Error',description:d.error||'No se pudo registrar',variant:'destructive'});
}
}catch{
toast({title:'Error',description:'Error de conexión',variant:'destructive'});
}finally{setPaying(false)}
}}>
{paying?<><Loader2 className="h-4 w-4 mr-2 animate-spin"/>Registrando...</>:'Registrar Cobro'}
</Button>
</div>
</DialogContent>
</Dialog>
