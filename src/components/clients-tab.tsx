...
const GB='bg-emerald-600 hover:bg-emerald-700 text-white border-0';
const sC=(s:number|null)=>{if(s===null)return{bg:'bg-background/70 dark:bg-card/80',tx:'text-foreground/70 dark:text-muted-foreground',bd:'border-input dark:border-emerald-500/5',lb:'Sin score',br:'bg-slate-400'};if(s<=30)return{bg:'bg-red-100 dark:bg-red-900/50',tx:'text-red-700 dark:text-red-300',bd:'border-red-200 dark:border-red-800',lb:'Malo',br:'bg-red-500'};if(s<=60)return{bg:'bg-amber-100 dark:bg-amber-900/50',tx:'text-amber-700 dark:text-amber-300',bd:'border-amber-200 dark:border-amber-800',lb:'Regular',br:'bg-amber-500'};return{bg:'bg-emerald-100 dark:bg-emerald-900/50',tx:'text-emerald-700 dark:text-emerald-300',bd:'border-emerald-200 dark:border-emerald-800',lb:'Bueno',br:'bg-emerald-500'}};
const lB=(st:string)=>{const m:Record<string,[string,string]>={active:['bg-emerald-100 dark:bg-emerald-900/50 text-emerald-800 dark:text-emerald-200 border-emerald-200 dark:border-emerald-800','Activo'],mora:['bg-red-100 dark:bg-red-900/50 text-red-800 dark:text-red-200 border-red-200 dark:border-red-800','Mora'],completed:['bg-teal-100 dark:bg-teal-900/50 text-teal-800 dark:text-teal-200 border-teal-200 dark:border-teal-800','Pagado'],paid:['bg-teal-100 dark:bg-teal-900/50 text-teal-800 dark:text-teal-200 border-teal-200 dark:border-teal-800','Pagado'],cancelled:['bg-background/70 dark:bg-card/80 text-foreground/70 dark:text-muted-foreground border-input dark:border-emerald-500/5','Cancelado'],refinanced:['bg-amber-100 dark:bg-amber-900/50 text-amber-800 dark:text-amber-200 border-amber-200 dark:border-amber-800','Refinanciado']};return m[st]||['bg-background/70 dark:bg-card/80 text-foreground/70 dark:text-muted-foreground border-input dark:border-emerald-500/5',st]};
const dSB=(t:string)=>{if(t==='dni'||t==='peruano')return['DNI','bg-blue-50 dark:bg-blue-950/50 text-blue-700 dark:text-blue-300 border-blue-200'];if(t==='carnet_extranjeria'||t==='extranjero')return['CE','bg-purple-50 dark:bg-purple-950/50 text-purple-700 dark:text-purple-300 border-purple-200'];if(t==='pasaporte')return['PAS','bg-amber-50 dark:bg-amber-950/50 text-amber-700 dark:text-amber-300 border-amber-200'];return['DOC','bg-background/50 dark:bg-card/80 text-foreground/70 dark:text-muted-foreground border-input']};
...
const[payClient,setPayClient]=useState<CWS|null>(null);
const[payLoans,setPayLoans]=useState<{id:string;status:string;amount:number;totalAmount:number;amountPaid:number}[]>([]);
const[payLoadingLoans,setPayLoadingLoans]=useState(false);
...
<div className="relative"><Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground"/><Input placeholder="Buscar..." value={search} onChange={e=>{setSearch(e.target.value);setPage(1)}} className="pl-9 w-full sm:w-64 bg-white dark:bg-[#05060b]/80 border-input"/></div>
<Select value={zoneF} onValueChange={v=>{setZoneF(v);setPage(1)}}><SelectTrigger className="w-full sm:w-44 bg-white dark:bg-[#05060b]/80 border-input"><MapPin className="h-4 w-4 mr-2 text-muted-foreground"/><SelectValue placeholder="Zonas"/></SelectTrigger><SelectContent><SelectItem value="all">Todas</SelectItem>{zones.map(z=><SelectItem key={z.id} value={z.id}>{z.name}</SelectItem>)}</SelectContent></Select>
</div>
<div className="flex items-center gap-2 w-full sm:w-auto">
<Button variant="outline" size="icon" className="border-input" onClick={fetchC}><RefreshCw className="h-4 w-4"/></Button>
...
:!clients.length?<Card className="border-0 shadow-md"><CardContent className="p-12 text-center"><div className="w-16 h-16 rounded-full bg-emerald-50 dark:bg-emerald-950/30 flex items-center justify-center mx-auto mb-4"><Users className="h-8 w-8 text-emerald-400"/></div><h3 className="text-lg font-semibold text-foreground/80 dark:text-foreground/80 mb-2">No hay clientes</h3><p className="text-muted-foreground dark:text-muted-foreground mb-4">{search||zoneF!=='all'?'Sin resultados':'Registre el primer cliente'}</p><Button className={GB} onClick={openNew}><UserPlus className="h-4 w-4 mr-2"/>Registrar</Button></CardContent></Card>
...
<DialogHeader className="px-6 pt-6 pb-4 border-b border-input/50 dark:border-emerald-500/10 bg-gradient-to-r from-slate-50 to-white dark:from-slate-900 dark:to-slate-900">
...
{/* Doc Number */}
<div className="space-y-2">
<Label htmlFor="dn" className="text-sm font-semibold text-foreground/80 dark:text-foreground/80">Número de {curD.lb} <span className="text-red-500">*</span></Label>
...
{vRes.found&&<div className="p-2 space-y-1.5 max-h-36 overflow-y-auto bg-white dark:bg-[#05060b]/80">
{vRes.results.clients.map(cr=><div key={cr.id} className="flex items-center gap-2 p-2 bg-background/50 dark:bg-card/70 rounded-md text-xs"><User className="h-3.5 w-3.5 text-emerald-600 dark:text-emerald-300"/><span className="font-medium truncate">{cr.name}</span><span className="text-muted-foreground dark:text-muted-foreground">{cr.phone}</span><Badge variant="outline" className={`text-[10px] ml-auto ${cr.hasActiveLoans?'bg-amber-50 dark:bg-amber-950/50 text-amber-700 dark:text-amber-300 border-amber-200':'bg-emerald-50 dark:bg-emerald-950/50 text-emerald-700 dark:text-emerald-300 border-emerald-200'}`}>{cr.hasActiveLoans?'Con préstamo':'Cliente'}</Badge></div>)}
{vRes.results.staff.map(sr=><div key={sr.id} className="flex items-center gap-2 p-2 bg-background/50 dark:bg-card/70 rounded-md text-xs"><Shield className="h-3.5 w-3.5 text-purple-600 dark:text-purple-300"/><span className="font-medium truncate">{sr.name}</span><span className="text-muted-foreground dark:text-muted-foreground">{sr.phone}</span><Badge variant="outline" className="text-[10px] ml-auto bg-purple-50 dark:bg-purple-950/50 text-purple-700 dark:text-purple-300 border-purple-200">{sr.role}</Badge></div>)}
</div>}</div>}
</div>
{/* Name */}
<div className="space-y-2">
<Label htmlFor="nm" className="text-sm font-semibold text-foreground/80 dark:text-foreground/80">Nombre completo <span className="text-red-500">*</span></Label>
<div className="relative">                              <Input id="nm" placeholder="Juan Pérez García" value={fName} onChange={e=>setFName(e.target.value)} className={`bg-white dark:bg-[#05060b]/80 h-11 pr-10 ${fName?(nOk?'border-emerald-400 focus-visible:ring-emerald-400':'border-red-400 focus-visible:ring-red-400'):''}`} disabled={editing}/>{fName&&<div className="absolute right-3 top-1/2 -translate-y-1/2">{nOk?<CheckCircle2 className="h-4 w-4 text-emerald-500"/>:<XCircle className="h-4 w-4 text-red-400"/>}</div>}</div>
</div>
{/* Phone */}
<div className="space-y-2">
<Label htmlFor="ph" className="text-sm font-semibold text-foreground/80 dark:text-foreground/80">Teléfono <span className="text-red-500">*</span></Label>
<div className="flex items-center gap-2">
<div className="relative flex-1">
...
{/* Address */}
<div className="space-y-2">
<Label htmlFor="ad" className="text-sm font-semibold text-foreground/80 dark:text-foreground/80">Dirección <span className="text-muted-foreground font-normal">(opcional)</span></Label>
<Input id="ad" placeholder="Av. Principal 123" value={fAddr} onChange={e=>setFAddr(e.target.value)} className="bg-white dark:bg-[#05060b]/80 h-11"/>
</div>
{/* Zone */}
<div className="space-y-2">
<Label className="text-sm font-semibold text-foreground/80 dark:text-foreground/80">Zona <span className="text-muted-foreground font-normal">(opcional)</span></Label>
<Select value={fZone} onValueChange={setFZone}><SelectTrigger className="bg-white dark:bg-[#05060b]/80 border-input h-11"><MapPin className="h-4 w-4 mr-2 text-muted-foreground"/><SelectValue placeholder="Seleccionar zona"/></SelectTrigger><SelectContent>{zones.map(z=><SelectItem key={z.id} value={z.id}>{z.name}</SelectItem>)}</SelectContent></Select>
</div>
{/* Collector */}
<div className="space-y-2">
<Label className="text-sm font-semibold text-foreground/80 dark:text-foreground/80">Cobrador asignado <span className="text-muted-foreground font-normal">(opcional)</span></Label>
<Select value={fCollector} onValueChange={setFCollector}><SelectTrigger className="bg-white dark:bg-[#05060b]/80 border-input h-11"><User className="h-4 w-4 mr-2 text-muted-foreground"/><SelectValue placeholder="Sin cobrador asignado"/></SelectTrigger><SelectContent>{collectors.filter(c=>c.name).map(c=><SelectItem key={c.id} value={c.id}>{c.name}</SelectItem>)}</SelectContent></Select>
</div>
{/* Score */}
<div className="space-y-3">
<div className="flex items-center justify-between"><Label className="text-sm font-semibold text-foreground/80 dark:text-foreground/80">Score crediticio</Label><Badge variant="outline" className={`${sC(fScore[0]).bg} ${sC(fScore[0]).tx} ${sC(fScore[0]).bd} font-semibold`}>{fScore[0]} — {sC(fScore[0]).lb}</Badge></div>
<Slider value={fScore} onValueChange={setFScore} min={0} max={100} step={1}/>
<div className="flex justify-between text-xs text-muted-foreground"><span>0</span><span>30</span><span>60</span><span>100</span></div>
</div>
</div>
</ScrollArea>
<DialogFooter className="px-6 py-4 border-t border-input/50 dark:border-emerald-500/10 bg-gradient-to-r from-slate-50 to-white dark:from-slate-900 dark:to-slate-900">
<Button variant="outline" onClick={()=>{setFOpen(false);reset()}} disabled={saving} className="border-input">Cancelar</Button>
...
<AlertDialogAction className="bg-red-600 hover:bg-red-700 text-white shadow-lg shadow-red-600/25" onClick={del} disabled={deling||(delC?.stats.activeLoans??0)>0}>{deling?<><Loader2 className="h-4 w-4 mr-2 animate-spin"/>Eliminando...</>:'Eliminar'}</AlertDialogAction>
</AlertDialogFooter>
</AlertDialogContent>
</AlertDialog>
...
<SheetHeader className="pb-4 border-b border-input/50 dark:border-emerald-500/10">
<div className="flex items-center gap-3">{c.photoUrl?<img src={c.photoUrl} alt={c.name} className="w-12 h-12 rounded-full object-cover border-2 border-emerald-200"/>:<div className={`w-12 h-12 rounded-full flex items-center justify-center ${ab}`}><User className={`h-6 w-6 ${at}`}/></div>}<div><SheetTitle className="text-lg">{c.name}</SheetTitle><SheetDescription className="flex items-center gap-2 mt-0.5"><Badge variant="outline" className={`text-[10px] px-1.5 py-0 h-5 ${dc}`}>{dl}</Badge>{dn}{c.status!==null&&<Badge variant="outline" className={`text-[10px] px-1.5 py-0 h-5 ${c.status==='active'?'bg-emerald-50 dark:bg-emerald-950/50 text-emerald-700 dark:text-emerald-300 border-emerald-200':'bg-background/70 dark:bg-card/80 text-muted-foreground dark:text-muted-foreground border-input'}`}>{c.status==='active'?'Activo':'Inactivo'}</Badge>}</SheetDescription></div></div>
</SheetHeader>
<div className="p-4 space-y-5">
<Card className="border-0 shadow-sm"><CardContent className="p-4">
<div className="flex items-center justify-between mb-2"><span className="text-sm font-medium text-foreground/80 dark:text-foreground/80">Score</span><Badge variant="outline" className={`font-bold ${sc.bg} ${sc.tx} ${sc.bd}`}>{c.creditScore??'N/A'} — {sc.lb}</Badge></div>
{c.creditScore!==null&&<><Progress value={c.creditScore} className={`h-3 [&>div]:${sc.br}`}/><div className="flex justify-between text-xs text-muted-foreground"><span>0</span><span>30</span><span>60</span><span>100</span></div></>}
{c.creditScoreLabel&&<div className="mt-2 text-xs text-muted-foreground dark:text-muted-foreground text-center">Etiqueta: <span className="font-semibold text-foreground/80 dark:text-foreground/80">{c.creditScoreLabel}</span></div>}
</CardContent></Card>
<div className="space-y-2">
<h3 className="text-sm font-semibold text-foreground/80 dark:text-foreground/80 flex items-center gap-2"><Phone className="h-4 w-4 text-emerald-500"/>Información</h3>
<div className="flex items-center gap-3 p-3 bg-background/50 dark:bg-card/70 rounded-lg"><IdCard className="h-4 w-4 text-muted-foreground"/><div><p className="text-xs text-muted-foreground">Documento</p><p className="text-sm font-medium">{dl} {dn}</p></div></div>
<div className="flex items-center gap-3 p-3 bg-background/50 dark:bg-card/70 rounded-lg"><Phone className="h-4 w-4 text-muted-foreground"/><div><p className="text-xs text-muted-foreground">Teléfono</p><p className="text-sm font-medium">{c.phone}</p></div></div>
{c.address&&<div className="flex items-center gap-3 p-3 bg-background/50 dark:bg-card/70 rounded-lg"><MapPin className="h-4 w-4 text-muted-foreground"/><div><p className="text-xs text-muted-foreground">Dirección</p><p className="text-sm font-medium">{c.address}</p></div></div>}
{c.zone&&<div className="flex items-center gap-3 p-3 bg-background/50 dark:bg-card/70 rounded-lg"><MapPin className="h-4 w-4 text-muted-foreground"/><div><p className="text-xs text-muted-foreground">Zona</p><p className="text-sm font-medium">{c.zone.name}</p></div></div>}
<div className="flex items-center gap-3 p-3 bg-background/50 dark:bg-card/70 rounded-lg"><Calendar className="h-4 w-4 text-muted-foreground"/><div><p className="text-xs text-muted-foreground">Registro</p><p className="text-sm font-medium">{fD(c.createdAt)}</p></div></div>
{c.collectorId&&<div className="flex items-center gap-3 p-3 bg-background/50 dark:bg-card/70 rounded-lg"><User className="h-4 w-4 text-muted-foreground"/><div><p className="text-xs text-muted-foreground">Cobrador ID</p><p className="text-sm font-medium font-mono text-xs">{c.collectorId.slice(0,8)}...</p></div></div>}
</div>
<Separator/>
<div className="space-y-3">
<h3 className="text-sm font-semibold text-foreground/80 dark:text-foreground/80 flex items-center gap-2"><TrendingUp className="h-4 w-4 text-emerald-500"/>Financiero</h3>
...
<div className="p-3 bg-background/50 dark:bg-card/70 rounded-lg text-center border border-input/50 dark:border-emerald-500/5"><p className="text-xs text-foreground/70 dark:text-muted-foreground">Préstamos</p><p className="text-lg font-bold text-foreground/80 dark:text-foreground/80">{c.stats.totalLoans}</p></div>
...
{c.guarantors.length>0&&<><Separator/><div className="space-y-2"><h3 className="text-sm font-semibold text-foreground/80 dark:text-foreground/80 flex items-center gap-2"><Shield className="h-4 w-4 text-emerald-500"/>Avalistas ({c.guarantors.length})</h3>{c.guarantors.map(g=><div key={g.id} className="flex items-center gap-3 p-3 bg-background/50 dark:bg-card/70 rounded-lg"><div className="w-8 h-8 rounded-full bg-amber-100 dark:bg-amber-900/50 flex items-center justify-center"><User className="h-4 w-4 text-amber-600 dark:text-amber-300"/></div><div><p className="text-sm font-medium">{g.name}</p>{g.phone&&<p className="text-xs text-muted-foreground dark:text-muted-foreground">{g.phone}</p>}</div></div>)}</div></>}
{c.loans.length>0&&<><Separator/><div className="space-y-2"><h3 className="text-sm font-semibold text-foreground/80 dark:text-foreground/80 flex items-center gap-2"><CreditCard className="h-4 w-4 text-emerald-500"/>Préstamos ({c.loans.length})</h3>{c.loans.map(l=>{const[cn,lb]=lB(l.status),pct=l.totalAmount>0?Math.round((l.amountPaid/l.totalAmount)*100):0;const isActive=l.status==='active'||l.status==='mora';return<div key={l.id} className="p-3 bg-background/50 dark:bg-card/70 rounded-lg space-y-2"><div className="flex items-center justify-between"><Badge variant="outline" className={`text-xs ${cn}`}>{lb}</Badge><span className="text-sm font-bold">{fC(l.amount)}</span></div><div className="flex justify-between text-xs text-muted-foreground dark:text-muted-foreground"><span>Pagado: {fC(l.amountPaid)}</span><span>{pct}%</span></div><Progress value={pct} className="h-1.5"/>{isActive&&<Button className="w-full mt-1 h-7 text-xs bg-emerald-600 hover:bg-emerald-700 text-white" onClick={()=>{setDetO(false);window.dispatchEvent(new CustomEvent('navigate-to-tab',{detail:{tab:'payments'}}))}}>Cobrar</Button>}{l.payments&&l.payments.length>0&&<div className="mt-1 pt-2 border-t border-input dark:border-emerald-500/5 space-y-1">{l.payments.map(p=><div key={p.id} className="flex items-center justify-between text-xs"><span className="text-muted-foreground">{fDT2(p.paymentDate)}</span><span className="font-semibold text-emerald-600 dark:text-emerald-400">{fC(p.amount)}</span><Badge variant="outline" className="text-[10px] bg-white dark:bg-[#05060b]/80">{PAYMENT_METHOD_LABELS[p.paymentMethod]||p.paymentMethod}</Badge></div>)}</div>}</div>})}</div></>}
<div className="flex gap-3 pt-2">
<Button className={`flex-1 ${GB}`} onClick={()=>{setDetO(false);openEdit(c)}}><Pencil className="h-4 w-4 mr-2"/>Editar</Button>
<Button variant="outline" className="border-input" onClick={()=>setNotesO(true)}><StickyNote className="h-4 w-4 mr-2"/>Notas</Button>
<Button variant="outline" className="flex-1 border-input" onClick={()=>setDetO(false)}>Cerrar</Button>
</div>
</div>
</>);})()}
</SheetContent>
...
open={payOpen}
onOpenChange={o=>{if(!o)setPayClient(null);setPayOpen(o)}}
preselectedClientId={payClient?.id}
preselectedClientName={payClient?.name}
onSuccess={fetchC}
/>
</div>);
}
