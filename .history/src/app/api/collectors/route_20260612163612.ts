...
export async function POST(request: NextRequest) {
  try {
    ...
    if (isVercel) {
      ...
      if (documentNumber) {
        const { data: existingDoc } = await supabase.from('profiles').select('id').eq('document_number', documentNumber).maybeSingle();
        if (existingDoc) {
      ...
      return NextResponse.json({
        ...
        documentNumber: newProfile.document_number || null,
        photoUrl: null,
  ...
  try {
    ...
    if (isVercel) {
      ...
      return NextResponse.json({
        ...
        documentNumber: updatedProfile.document_number || null,
        photoUrl: null,
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    if (isVercel) {
      ...
      // Check existing via Supabase
      const { data: profile, error: findError } = await supabase.from('profiles').select('id, name, document_type, document_number').eq('id', id).maybeSingle();
    ...
    for (const profile of profiles) {
      await db.profile.upsert({
        ...
        update: {
          ...
          documentNumber: (profile.document_number as string) || null,
